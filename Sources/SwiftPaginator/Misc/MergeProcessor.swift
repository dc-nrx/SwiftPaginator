...
public struct MergeProcessor<Item> {
	...
	public init(
		...
		self.preprocessInput = pre
		self.merge = merge
		self.postprocessResult = post
	}
	
	static func dropSameIDs(prioritizeNewlyFetched: Bool) -> MergeProcessor where Item: Identifiable {
		//TODO: add IDs cache
